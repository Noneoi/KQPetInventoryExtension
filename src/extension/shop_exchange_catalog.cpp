...
QVector<int> intList(const QJsonValue& value) {
  ...
  for (const QJsonValue& item : value.toArray()) {
    int id = 0;
    if (!checkedCount(item, &id) || id <= 0) return {};
...
std::shared_ptr<const ShopCatalogSnapshot> ShopExchangeCatalog::prepare(const QJsonObject& root, const QString& source,
  ...
  for (const QJsonValue& shopValue : root.value(QStringLiteral("shops")).toArray()) {
    ...
    if (!checkedCount(shopObject.value(QStringLiteral("shopId")), &shop.shopId) ||
    ...
    for (const QJsonValue& goodValue : shopObject.value(QStringLiteral("goods")).toArray()) {
      ...
      if (!checkedCount(goodObject.value(QStringLiteral("itemServerId")),
      ...
      if (goodObject.contains(QStringLiteral("tab")) && !checkedCount(goodObject.value(QStringLiteral("tab")), &good.tab)) return invalid();
      ...
      if (!checkedCount(goodObject.value(QStringLiteral("limitCount")),
...
QJsonObject ShopExchangeCatalog::parseOfficialText(const QString& text,
  ...
  while (markers.hasNext()) {
    ...
    while (matches.hasNext()) {
      ...
      if (limitParts.size() != 2 || !checkedCount(limitParts.value(0), &limitIndex) ||
          !checkedCount(limitParts.value(1), &limitCount) || limitIndex >= limitKeys.size()) {
      ...
      if (!tabText.isEmpty() && !checkedCount(tabText, &tab)) tab = -1;
