...
void RoutineOverviewController::revokeAsyncRequests() {
...
void RoutineOverviewController::pollAsyncRequests() {
  ...
  for (auto deadline = deadlines.begin(); deadline != deadlines.end(); ++deadline)
    if (deadline.value() <= now)
      completeRequest(deadline.key(), false, QStringLiteral("%1响应超时").arg(deadline.key()));
...
bool RoutineOverviewController::requestRefresh() {
  ...
  const auto sendRequest = [this](const QString& service, const QString& command,
    ...
    if (!asyncSender_) requestDispatchTimes_.insert(command, transportMonotonicMs());
