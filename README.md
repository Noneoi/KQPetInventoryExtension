...
当前仓库为 **3.0.0 源码**。已有开发构建通过完整 Release 回归（73 项）；真实客户端交互由用户手动验证。二进制的具体版本、源码摘要和测试结果以对应产物的 identity 与报告为准。

## 安装与升级（推荐）

在 [v3.0.0 发行页](https://github.com/Noneoi/KQPetInventoryExtension/releases/tag/v3.0.0) 下载名称含 **copy-ready** 的复制即用 ZIP。
...
首次使用可在设置点击“全部检查更新”（也可以按类别单独更新）；商店中的“刷新兑换次数”用于手动读取账号状态。
...
| 设置 → 全部检查更新 | 联网更新官方公共资料、兑换目录和图片资源定义；只下载官方版本有变化的部分 |
| 设置 → 单独更新某一类 | 只检查精灵与养成资料、兑换商店、精灵图片索引、星神与属性图标或日常与活动中的一类，其余保持不变 |
...
首次点击“全部检查更新”会准备便携 Python、Java 和 JPEXS 工具，可能需要几分钟；后续复用本地工具与同版本资源。公共资料更新不会代替账号查询。
