...
def self_test(root: Path) -> None:
    ...
    cases = [
        '#include "../extension/pet_repository.h"\n', '#include <QWidget>\n',
        '#include "../contracts/session_context.h"\n', '#include "../application/application_runtime.h"\n',
        '#include <windows.h>\n', '#include SOME_HEADER\n',
    ...
    assert inspect(contract_probe, root, '#include "../application/application_runtime.h"\n')
    assert inspect(contract_probe, root, 'QObject* hiddenRuntime;\n')


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--root", type=Path, default=Path(__file__).resolve().parents[1] / "src" / "domain")
