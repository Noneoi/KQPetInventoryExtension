...
PORTS = {
    "src/application/views/inventory_read_view.h", "src/application/views/analysis_read_view.h",
    "src/application/images/image_service.h",
    "src/contracts/refresh_timings.h", "src/contracts/operation_types.h",
...
VALUE_HELPERS = {
    "src/ui/common/pet_facts_ui.h", "src/diagnostics/build_info.h",
}
FORBIDDEN_APIS = {
...
def cmake_text(project: Path) -> str:
    """CMakeLists.txt plus the cmake/*.cmake fragments it include()s."""
    files = [project / "CMakeLists.txt", *sorted((project / "cmake").glob("*.cmake"))]
    return "\n".join(path.read_text(encoding="utf-8-sig") for path in files)


def check(project: Path):
    sources, errors = build_boundary(cmake_text(project))
...
def self_test(project: Path):
    base = "add_library(KQPetUi STATIC src/ui/shop/shop_window.cpp)\n"
    ...
    probe = project / "src/ui/ui_boundary_probe.cpp"
    allowed = '#include <QWidget>\n#include <QImage>\n#include <windows.h>\n'
    allowed += '#include "application/views/inventory_read_view.h"\n#include "domain/pet_era.h"\n'
    ...
    for source in [
        '#include "application/catalog/pet_detail_catalog.h"', '#include <pet_repository.h>',
        '#include "application/runtime/application_runtime.h"',
        '#include "application/pet/pet_refresh_controller.h"',
        '#include "application/catalog/shop_exchange_catalog.h"', '#include SECRET_HEADER',
        '#include "pet_detail_catalog.h"', '#include "../application/pet/pet_repository.h"',
    ...
    port = project / "src/application/views/inventory_read_view.h"
    assert inspect_source(port, project, '#include "application/runtime/application_runtime.h"', set())[1]
    assert "add_library(KQPetUi" in cmake_text(project), "KQPetUi target not found in CMake fragments"
